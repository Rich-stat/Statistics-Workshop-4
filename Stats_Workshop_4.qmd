...
<br> **1. Download the Excel file** [chi-sq goodness of fit.xlsx](https://kingstonuniversity-my.sharepoint.com/:x:/g/personal/ku11417_kingston_ac_uk/IQBsFJfG6pfLSbL2JAsDIqcGAdmJAUJQWTI4PIlHWRn6Pnk?e=1yJGcK) from Canvas and save it to a folder that you will use as your working directory (either in RStudio Cloud or in RStudio on your own laptop).

::: {.callout-note collapse="true" title="Upload excel file to RStudio Cloud" icon="true"}
- Check that the data in the file is 'tidy'
- Upload it to RStudio Cloud, saving it to a folder for this Statistics Workshop 4 session (*NB* If you are using RStudio on your own laptop, simply save the excel file to a relevant folder on your laptop).

![Uploading a file to RStudio](upload%20gof.png){.lightbox width="70%"}
...
::: {.callout-note collapse="true" title="Click to see the Cramer's V output" icon="false"}
```{r}
...
If you look at the interpretation tab below, you will see that the effect size is 'small'. (NB there are 4 categories here, so k=4)
:::

::: {.callout-note collapse="true" title="Cramer's V Interpretation" icon="false"}
![Cramer's V goodness of fit](Cramer's_V_goodness.png){width="700"}
:::
...
<br> **1. Download the Excel file** [titanic table data.xlsx](https://kingstonuniversity-my.sharepoint.com/:x:/g/personal/ku11417_kingston_ac_uk/IQCPKO6Av36uT7ZG9imVe4kIAfr8wcouVVmFEitQ6Bim-38?e=Jm0dVk) from Canvas and save it to a folder that you will use as your working directory (either in RStudio Cloud or in RStudio on your own laptop).

::: {.callout-note collapse="true" title="Upload excel file to RStudio Cloud" icon="true"}
- Check that the data in the file is 'tidy'
- Upload it to RStudio Cloud, saving it to a folder for this Statistics Workshop 4 session (*NB* If you are using RStudio on your own laptop, simply save the excel file to a relevant folder on your laptop).

![Uploading a file to RStudio](upload%20titanic.png){.lightbox width="70%"}
...
**Note**: this is a 2 x 2 contingency table; *ie*, there are 2 rows and 2 columns.
:::
...
The Odds ratio value depends on which category appears first for each factor in your contingency table.<br> <br>In this example, 'Died' appears first for the Survival factor; 'Male' appears first for the Gender factor.
...
`titanConTab |>`<br> `cramer_v()`<br> <br> Cramer's V ranges from 0 (no association between factors) to 1 (complete association). The tab below shows how to interpret the Cramer's V value obtained.<br>

::: {.callout-note collapse="true" title="Cramer's V Interpretation" icon="false"}
![Cramer's V contingency table](Cramer's_V_contingency.png){width="700"}

The above example is a 2 x 2 contingency table. If it had been a 3 x 2 table, k = 2, just as it is for a 2 x 2 table. If there were 3 rows and 4 columns, k = 3.
:::
...
<br> **1. Download the Excel file** [mcnemar data.xlsx](https://kingstonuniversity-my.sharepoint.com/:x:/g/personal/ku11417_kingston_ac_uk/IQD24Ggg08T-TIeSmC6NMhUZAbrFFxTqhcb0_G0oLHtGHd0?e=s3ouH7) from Canvas and save it to a folder that you will use as your working directory (either in RStudio Cloud or in RStudio on your own laptop).

::: {.callout-note collapse="true" title="Upload excel file to RStudio Cloud" icon="true"}
- Check that the data in the file is 'tidy'
- Upload it to RStudio Cloud, saving it to a folder for this Statistics Workshop 4 session (*NB* If you are using RStudio on your own laptop, simply save the excel file to a relevant folder on your laptop).

![Uploading a file to RStudio](upload%20mcnemar.png){.lightbox width="70%"}
